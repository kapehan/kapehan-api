const { feedback } = require("../db.service.js");
...
const findById = async (_id) => {
  const found = await feedback.findByPk(_id);
  if (!found) return null;
  return found.toJSON();
};

const update = async (_id, data) => {
  const [_, [updated]] = await feedback.update(data, {
    where: { _id },
    returning: true,
  });
...
const remove = async (_id) => {
  return await feedback.destroy({ where: { feedback_uuid:_id } });
};
